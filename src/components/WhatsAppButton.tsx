...
import { useSettings, whatsappNumber, DEFAULT_WHATSAPP } from "@/lib/site-data";
import { recordContact } from "@/lib/leads";

// Default fallback number (international format, no +). The live number is
// managed from the admin dashboard and stored in site settings.
export const WHATSAPP_NUMBER = DEFAULT_WHATSAPP;

export function WhatsAppButton() {
  const { t, dir } = useLanguage();
  const { data: settings } = useSettings();
  const href = `https://wa.me/${whatsappNumber(settings)}`;
  return (
    <a
      href={href}
      target="_blank"
